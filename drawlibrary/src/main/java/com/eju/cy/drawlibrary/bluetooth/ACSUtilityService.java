...
public class ACSUtilityService extends Service {
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            ...
            LogUtils.i(TAG, "Starting enable DATA Line Notificaiton");
		...
		public void onCharacteristicChanged(BluetoothGatt gatt,
			...
			if (characteristic.getUuid().equals(CMD_LINE_UUID)) {
				...
				if (value[0] == 0x05) {
                    LogUtils.d(TAG, "heart beat package write back ");
					/*
				...
				// 数据线
                LogUtils.d(TAG, "data line : length = " + value.length);
		...
		public void enablePhoneMode() {
            LogUtils.i(TAG, "Enabling Phone Mode...");
		...
		public void onCharacteristicWrite(BluetoothGatt gatt,
			...
			if (isInitializing && characteristic.getUuid().equals(CMD_LINE_UUID)) {
                ...
                LogUtils.i(TAG, "data has written!");
				...
				if (workerThread == null) {
                    LogUtils.e(TAG, "workerThread is null");
					return ;
				}
				if (status == BluetoothGatt.GATT_SUCCESS) {
				...
				synchronized (workerThread) {
                    LogUtils.d(TAG, "notify workerThread");
					wakeTimes++;
					workerThread.notify();
				}
			}
		...
		public void onDescriptorWrite(BluetoothGatt gatt,
            ...
            LogUtils.i(TAG, descriptor.getCharacteristic().getUuid() + " Notification Enabled");
			if (descriptor.getCharacteristic().getUuid().equals(DATA_LINE_UUID)) {
	...
	public boolean initialize() {
        ...
        // BluetoothManager.
        LogUtils.i(TAG, "Initializing " + TAG);
	...
	private boolean setACSNotification(UUID charaUUID) {
		...
		if (chara == null) {
            LogUtils.e(TAG, "Characteristic is null!");
            LogUtils.e(TAG, "Enableing Notification failed!");
			return false;
		}
	...
	private boolean writePackage(byte[] value){
        ...
        LogUtils.d(TAG, "writePort 4");
        LogUtils.d(TAG, "data: " + value.toString());
	...
	class WorkerThread extends Thread {
		...
		public void run() {
			...
			int count = 0;
			wakeTimes = 0;
            LogUtils.d(TAG, "There are " + packagesCount + " datas to be sended...");
			while (packagesCount > 0) {
                LogUtils.d(TAG, "sending data...count" + count);
				byte []blePackage = packages.get(count);
				writePackage(blePackage);
				//boolean isWait = false;
				synchronized (this) {
                    LogUtils.d(TAG, "synchronized...");
					try {
						if (wakeTimes == 0) {
                            LogUtils.d(TAG, "waiting...");
							// 设置超时
				...
				if (operationResult == 1) {
					...
					while (repeatSendTimes < 3) {
                        LogUtils.d(TAG, "repeat send data...count" + count);
						writePackage(blePackage);
						repeatSendTimes++;
						synchronized (this) {
							try {
		...
		public void run() {
			...
			try {
				...
				if (mPackagesCount == packagesCount) {
					workerThread.setOperationResult(0);
					synchronized (workerThread) {
                        LogUtils.d(TAG, "notify workerThread from timerThread!!!");
						wakeTimes++;
						workerThread.notify();
					}
				}
